...
export default function FinanceDashboard() {
  ...
  // 2. Fungsi untuk menarik data dari FastAPI (Filter Khusus Finance)
  const fetchLogs = async () => {
    try {
      ...
      if (dataPending.status === 'success') {
        // Hanya ambil data yang divisinya 'Finance'
        const financePending = dataPending.data.filter(log => log.category?.toLowerCase() === 'finance');
        setApprovalLogs(financePending);
      }

      const resHistory = await fetch('http://127.0.0.1:8000/api/approval-history');
      const dataHistory = await resHistory.json();
      if (dataHistory.status === 'success') {
        // Hanya ambil data history yang divisinya 'Finance'
        const financeHistory = dataHistory.data.filter(log => log.category?.toLowerCase() === 'finance');
  ...
  // 4. Update data statis (Disesuaikan untuk Finance)
  const stats = [
    { label: 'Total Accounts', value: '45', color: 'text-white' },
    { label: 'Active Ledger', value: '12', color: 'text-emerald-500' },
    { label: 'Pending Approval', value: approvalLogs.length.toString(), color: 'text-amber-500' },
  ];
      ...
      <div className="bg-slate-900/20 border border-white/5 rounded-[2.5rem] overflow-hidden">
        <div className="p-6 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
          <h3 className="text-[11px] font-black text-white uppercase tracking-widest">
            {activeTab === 'pending' ? 'Pending Transaction Logs' : 'Transaction History Logs'}
          </h3>
          
          <div className="flex gap-2">
             <button 
                onClick={() => setActiveTab('pending')}
                className={`w-2 h-2 rounded-full transition-all cursor-pointer ${activeTab === 'pending' ? 'bg-amber-500 animate-pulse scale-125' : 'bg-slate-700 hover:bg-slate-500'}`}
                title="View Pending Approvals"
             />
             <button 
                onClick={() => setActiveTab('history')}
                className={`w-2 h-2 rounded-full transition-all cursor-pointer ${activeTab === 'history' ? 'bg-amber-500 animate-pulse scale-125' : 'bg-slate-700 hover:bg-slate-500'}`}
        ...
        <div className="p-2 transition-all duration-300">
          {isLoading ? (
            <div className="p-10 text-center text-slate-500 text-sm animate-pulse">Scanning ledger...</div>
            ...
            <div className="p-10 text-center">
              ...
              <h4 className="text-white font-bold text-sm">
                {activeTab === 'pending' ? 'Ledger Clear' : 'No History Found'}
              </h4>
              <p className="text-[10px] text-slate-500 mt-1 max-w-xs mx-auto uppercase tracking-tighter">
                {activeTab === 'pending' 
                  ? 'No pending transactions in the queue. Ledger is fully synchronized.' 
                  : 'There are no previously processed transactions in the database.'}
              </p>
            </div>
          ) : (
